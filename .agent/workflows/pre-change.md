...
```bash
systemctl is-active sniper-armada.service && curl -s -o /dev/null -w "Dashboard: %{http_code}\n" http://localhost:3000
```
...
```bash
journalctl -u sniper-armada.service --since "15 sec ago" --no-pager | grep -c panic
```
