...
async fn async_main() -> Result<()> {
                    ...
                    if l2_active {
                        ...
                        if range > 5_000_000_000 {
                            tracing::info!(event = "vol_engine_deferred",
                                range_usd = range / sniper_types::PRICE_SCALE_I,
                                vol_grid_usd = new_grid as i64 / sniper_types::PRICE_SCALE_I,
                        ...
                        if range > 5_000_000_000 {
                            tracing::info!(event = "volatility_spike",
                                range_usd = range / sniper_types::PRICE_SCALE_I,
                                new_grid_usd = new_grid as i64 / sniper_types::PRICE_SCALE_I);
                        }
                    }
                } // if let min/max
                } else {
    ...
    loop {
        ...
        let _ = mdata_read.write_frame(fastwebsockets::Frame::text(Payload::Owned(json!({"event":"subscribe","channel":"book","symbol":sniper_types::TRADING_SYMBOL,"prec":"P0","freq":"F0","len":"25"}).to_string().into_bytes()))).await;
                                            ...
                                            if best_bid > 0 && best_ask > 0 {
                                                ...
                                                if now.duration_since(last_upd).as_millis() > fire_interval as u128
                                                    && risk.paused.load(Ordering::Acquire) == 0 {
                                                        ...
                                                        let depth_btc = total_depth / sniper_types::PRICE_SCALE;
                                                        ...
                                                        if in_liquidity_hole && !was_in_hole {
                                                            tracing::warn!(event = "liquidity_hole",
                                                                ...
                                                                avg_depth_btc = format!("{:.4}", avg_depth / sniper_types::PRICE_SCALE),
                                                        ...
                                                        let delta_shift = if delta_active {
                                                            ...
                                                            tracing::info!(event = "delta_reposition",
                                                                ...
                                                                shift_usd = format!("{:.2}", shift_i as f64 / sniper_types::PRICE_SCALE),
                                                        ...
                                                        if buy_i >= ba_i {
                                                            buy_i = ba_i - MIN_TICK;
                                                            info!(event = "anti_cross_guard", side = "buy", clamped_to = buy_i as f64 / sniper_types::PRICE_SCALE, best_ask = ba_i as f64 / sniper_types::PRICE_SCALE);
                                                        }
                                                        if sell_i <= bb_i {
                                                            sell_i = bb_i + MIN_TICK;
                                                            info!(event = "anti_cross_guard", side = "sell", clamped_to = sell_i as f64 / sniper_types::PRICE_SCALE, best_bid = bb_i as f64 / sniper_types::PRICE_SCALE);
                                                        }
                                                        // Spread integrity: if inverted after clamping, skip cycle
                                                        if buy_i >= sell_i {
                                                            info!(event = "spread_inverted", buy = buy_i as f64 / sniper_types::PRICE_SCALE,
                                                                  sell = sell_i as f64 / sniper_types::PRICE_SCALE, grid = grid, bias = final_bias);
                                                            continue;
                                                        }
                                                        ...
                                                        if db >= MIN_TICK || ds >= MIN_TICK || lb == 0 {
                                                            ...
                                                            let r_pnl = eng.realized_pnl.load(Ordering::Acquire) as f64 / sniper_types::PRICE_SCALE;
                                                            ...
                                                            let w_btc = eng.wallet_btc.load(Ordering::Relaxed) as f64 / sniper_types::PRICE_SCALE;
                                                            let w_usd = eng.wallet_usd.load(Ordering::Relaxed) as f64 / sniper_types::PRICE_SCALE;
                                                            ...
                                                            info!(event = "hydra_fire", bid = best_bid, ask = best_ask,
                                                                  ...
                                                                  obi = format!("{:.3}", obi),
                                                                  order_usd = final_order_usd as f64 / sniper_types::PRICE_SCALE,
