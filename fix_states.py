...
# Paused is offset 0
write_u32('/dev/shm/sniper/risk_state.bin', 0, 0) # paused = 0

# Grid Risk State (global_paused is offset 8*8 = 64)
write_u64('/dev/shm/sniper/grid_risk.bin', 64, 0)

# Moonshot Risk State (global_paused is offset 16)
write_u64('/dev/shm/sniper/moonshot_risk.bin', 16, 0)

# Trigon Risk State (global_paused is offset 16)
write_u64('/dev/shm/sniper/trigon_risk.bin', 16, 0)

# Nexus Cross Exchange State (paper_mode is offset 4, emergency_pause is offset 8)
write_u32('/dev/shm/sniper/cross_exchange.bin', 4, 1) # paper=1
write_u32('/dev/shm/sniper/cross_exchange.bin', 8, 0) # paused=0

# Seed L2 Command with 0.5 ranging and 0.5 trending to wake up Kelly limits
with open('/dev/shm/sniper/l2_command.bin', 'r+b') as f:
    mm = mmap.mmap(f.fileno(), 0)
